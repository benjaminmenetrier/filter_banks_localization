...
\end{itemize}
where $\widehat{n}_p$ is the space size of component $p$, and $\displaystyle \widehat{n} = \sum_{p=1}^P n_p$ is the total size.\\
$  $\\
...
Often in practice, only low-pass filters are actually implemented, through a diffusion process or an equivalent smoother. However, the high-pass filter $\mathbf{G}^h_p$ can be built as the complement of the low-pass filter $\mathbf{G}^l_p$:
\begin{align}
\mathbf{G}^h_p = \mathbf{I}_n - \mathbf{G}^l_p
\end{align}
But we should notice that in this case:
\begin{itemize}
\item All the components must have the same grid size $\widehat{n}_p = n$, so the size of the transformed space is $\widehat{n} = Pn$.
\item It is not possible to benefit from interpolations: $\mathbf{S}_p = \mathbf{I}_n$
\end{itemize}
...
\begin{center}
\includegraphics[width=\linewidth]{filters_equivalence.pdf}
