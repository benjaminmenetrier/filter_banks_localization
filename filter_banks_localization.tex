...
The standard EnVar formulation can be generalized by introducing two linear transforms $\mathbf{F} \in \mathbb{R}^{\widehat{n} \times n}$ and $\mathbf{F}^\dag \in \mathbb{R}^{n \times \widehat{n}}$:
\begin{align}
\widehat{\mathbf{B}} & = \mathbf{F}^\dag \left(\mathbf{F} \left(\mathbf{X} \mathbf{X}^\mathrm{T}\right) \mathbf{F}^\mathrm{T} \circ \widehat{\mathbf{L}}\right) \left(\mathbf{F}^\dag\right)^\mathrm{T} \\
\delta \mathbf{x} & = \mathbf{F}^\dag \sum_{j=1}^N \left(\mathbf{F} \mathbf{X}_{:j}\right) \circ \left(\widehat{\mathbf{U}} \widehat{\mathbf{v}}_j\right)
\end{align}
where: 
\begin{itemize}
...
\lim_{N \rightarrow \infty} \widehat{\mathbf{B}} & = \left(\mathbf{F}^\dag \mathbf{F} \mathbf{X}\right) \left(\mathbf{F}^\dag \mathbf{F} \mathbf{X}\right)^\mathrm{T}
\end{align}
...
so
\begin{align}
\mathbf{F}^\dag \mathbf{F} \mathbf{X} = \mathbf{X}
\end{align}
Thus, $\mathbf{F}^\dag$ should be a left-inverse of $\mathbf{F}$ in the ensemble perturbations space.
\end{itemize}

The content of $\mathbf{F}$ can be very diverse:
\begin{itemize}
...
\end{itemize}
The components of $\mathbf{F}$ are built recursively:
\begin{itemize}
...
Formally, the linear transform $\mathbf{F}$ can be written as:
\begin{align}
...
\end{itemize} 
The left-inverse of $\mathbf{F}$ is simply:
\begin{align}
\mathbf{F}^\dag = \left(\mathbf{S}_1 \mathbf{S}_2 \dots \mathbf{S}_{P-1} \mathbf{I}_n\right)
\end{align}
...
$  $\\
Here, the SDL uses the \texttt{scipy.ndimage.gaussian\_filter} function of \texttt{Python} for $\mathbf{F}_p$. The IFBL uses a standard bilinear interpolation. The filtering scales of the SDL and the low-resolution grids of the IFBL have been briefly tuned to yield visually similar results.
